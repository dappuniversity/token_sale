...
function transfer() {
    ...
    const toAddress = '0x92E5A835bc90ABc6DA111Ad1309afD7e21d5b0d4';
    ...
    contract.methods.approve(fromAddress, 1).send({from: fromAddress}).on('transactionHash', function(hash) {
        console.log('approve: ' + hash);
    }).catch(console.error);
    contract.methods.transferFrom(fromAddress, toAddress, 1).send({from: fromAddress}).on('transactionHash', function(hash) {
        console.log('transferFrom: ' + hash);
    }).catch(console.error);
